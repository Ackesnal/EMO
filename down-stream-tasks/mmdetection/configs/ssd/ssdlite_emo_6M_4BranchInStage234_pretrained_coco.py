...
model = dict(
    ...
    backbone=dict(
        ...
        downsample_skip=False, conv_branchs=[False, True, False, True], shuffle=False,
        sync_bn=True, out_indices=(3, 4),
        pretrained='./runs_emo/CLS_EMO_6M_4BranchInStage234_DefaultCLS_Full_Trained_Results/net_E.pth',
        frozen_stages=-1, norm_eval=True),
    neck=dict(
        type='SSDNeck',
        in_channels=(160, 320),
        out_channels=(160, 320, 512, 256, 256, 128),
    ...
    bbox_head=dict(
        type='SSDHead',
        in_channels=(160, 320, 512, 256, 256, 128),
        num_classes=80,
        use_depthwise=True,
        norm_cfg=dict(type='BN', eps=0.001, momentum=0.03),
        act_cfg=dict(type='ReLU6'),
